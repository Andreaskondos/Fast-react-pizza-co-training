      ...
      <Form method="POST">
        ...
        >
          ...
          {!position.latitude && !position.longitude && (
            <span className="absolute -top-[3px] right-0 z-50 sm:right-[3px] sm:top-[3px] md:right-[5px] md:top-[5px]">
              <Button
                disabled={isAddressLoading}
                type="small"
                onClick={(e) => {
...
export async function action({ request }) {
  ...
  if (!isValidPhone(order.phone))
    errors.phone = "Please give us your phone number. We might need it";
  if (Object.keys(errors).length > 0) return errors;

  const newOrder = await createOrder(order);

  //Do NOT overuse this hack / trick cause it might cause some perfomance isues
